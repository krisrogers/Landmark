...
- `npm start` — Expo dev server (requires a phone with Expo Go; not usable in CI/remote sessions)
...
- Must keep working in **Expo Go** (the user's dev loop is QR-scan on a physical Android phone). Don't add packages that require a development build without discussing it first. Notably: background location, expo-maps, and most third-party native modules are NOT available in Expo Go.
